...
static int check_frontend (int fe_fd, int dvr, int human_readable)
{
	...
	do {
		...
		if (human_readable) {
			fInfo ("status %02x | signal %3u%% | snr %3u%% | ber %d | unc %d | ",
				   status, (signal * 100) / 0xffff, (snr * 100) / 0xffff, ber, uncorrected_blocks);
		} else {
			fInfo ("status %02x | signal %04x | snr %04x | ber %08x | unc %08x | ",
...
int DVBUtils::zapTo(QString name)
{
	...
	fInfo("LNB polarity has changed to %s\n", qPrintable(f[2]));
...
bool DVBUtils::tuneToChannel(QString ch)
{
	...
	if (err == -ENOENT) {
		fInfo("no such channel");
		return false;
	}
	if (err == -EINVAL) {
		fInfo("wrong channel parameters");
		return false;
	}
	...
	fInfo("tuned channel to %s", qPrintable(ch));
...
{
	fDebug("creating chlist for Turksat packet %s\n", qPrintable(packet));
	QStringList entries;
	...
	if (xml.hasError())
		fDebug("%s", qPrintable(xml.errorString()));
