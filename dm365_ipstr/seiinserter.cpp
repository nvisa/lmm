...
int SeiInserter::setAlarmInformation(const QString &templateFile, int alg, int gpioId, int ioLevel, int minDuration, int motThreshold)
{
	...
	info.motionAlarmThreshold = motThreshold;
	if (gpioId >= 0)
		addGpioAlarm(gpioId);
	return 0;
}
...
const QByteArray SeiInserter::generateAlarm()
{
	...
	if (info.algorithm == 1) {
		...
		if (canTrigger) {
			...
			} else if (info.current.active && info.current.source == 0) {
		...
		ba = info.alarmTemplate
				 ...
				 */
				.arg(info.current.active)
				.arg(info.current.id)
				.arg(info.current.source)
				.arg(motv)
				.arg(info.motionAlarmThreshold)
				.arg(ioState)

				.toUtf8();

		//qDebug() << info.current.active << info.current.id << info.current.source << motv << info.motionAlarmThreshold << ioState;
